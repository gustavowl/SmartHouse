...
import device.ArcadeMachine;
import device.BonusCard;
...
public class AddDevice extends JFrame {
	...
	public static void main(String[] args) {
		EventQueue.invokeLater(new Runnable() {
			public void run() {
				try {
					AddDevice frame = new AddDevice(new DeviceUI());
	 ...
	 * Create the frame.
	 */
	public AddDevice(javax.swing.JFrame deviceUI) {
		this.jframe = deviceUI;
		...
		Device arcademachine = new ArcadeMachine();
		Device card = new BonusCard();
		...
		devicesNaRede.add(arcademachine);
		devicesNaRede.add(card);
	...
	public void addDevice() {
		...
		//novoDevice = textField.getText();
		telaFacade.setOpcao(comboBox.getSelectedIndex() + 1);
		telaFacade.addConnectedDevice(textField.getText());
	...
	public void addDeviceComboBox(String deviceNome) {
		comboBox.addItem(deviceNome);
	}

	public TelaFacade getTelaFacade() {
		return telaFacade;
	}

	public void setTelaFacade(TelaFacade telaFacade) {
		this.telaFacade = telaFacade;
	}
	
	/*public void removeDeviceComboBox(String deviceNome) {
		int index = 0;
		for(String nome : comboBox.get) {
			if(comboBox.getSelectedItem().equals(deviceNome)) {
